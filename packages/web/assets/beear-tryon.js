/* @beear/tryon v0.4.0 — IIFE for web + Android WebView
 ...
 */
const REFERENCE_CARD_WIDTH_MM = 85.6;
 ...
 * @param {number} referenceMm - known width of the reference object in mm (default: credit card width)
 * @param {number} facePx - measured inter-pupil distance in pixels
 * @returns {{ pdMm: number, pxPerMm: number, ok: boolean }}
 */
function calibrateFromReference(referencePx, referenceMm = REFERENCE_CARD_WIDTH_MM, facePx) {
 ...
 */
function estimatePd(facePx, pxPerMm) {
...
function drawGlassesOverlay(ctx, face, selectedA, selectedB, compareMode, pdMm) {
  ...
  var api = {
    ...
    REFERENCE_CARD_WIDTH_MM: REFERENCE_CARD_WIDTH_MM,
    calibrateFromReference: calibrateFromReference,
    estimatePd: estimatePd,
